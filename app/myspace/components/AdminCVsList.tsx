...
import {Add, Close, Delete, Edit, OpenInNew, Preview,} from "@mui/icons-material";
import {
    Avatar,
    Button,
    Container,
...
export default function AdminMyCvsList() {
    const columns: GridColDef<(typeof rows)[number]>[] = [

        {
            ...
            renderCell: (value) => (
                <Avatar
                    src={`${process.env.NEXT_PUBLIC_ONLINE_WS_URI}/myfiles/cvphoto/${value?.value}`}
    ...
    const [editMyCv, setEditMyCv] = React.useState(null);
    ...
    React.useEffect(() => {
        const getStories = async () => {
            ...
            await axiosApi
                .get(`/mycvs?page=${currentPage}&size=${pageSize}`, {})
        ...
        getStories();
    }, [setLoading, page, update]);

    const deleteMyCv = async (slug: any) => {
        await axiosApi
            .delete(`/mycvs/${slug}`)
            .then((reponse) => reponse.data)
            .then((json) => {
        ...
        <Box sx={{height: "100%", width: "100%"}}>
            ...
            <Dialog maxWidth="md" fullWidth sx={{}} open={open} onClose={handleClose}>
                <DialogTitle
                    ...
                    alignItems={"center"}
                >
                    <Typography fontWeight={600}>{openSlug?.title}</Typography>{" "}
